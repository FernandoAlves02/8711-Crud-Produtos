...
class Cliente_Terminal_View:
    ...
    def ler_dados_cliente(self, cliente_existente=None):
        ...
        data_nascimento = self.ler_campo("Data de nascimento (dd/mm/aaaa)", Data_Utils.data_para_sting(cliente_existente.data_nascimento) if cliente_existente else None)
        limite_credito = float(self.ler_campo("Limite de crédito", cliente_existente.limite_credito if cliente_existente else None))
    ...
    def exibir_clientes(self, clientes):
        ...
        print("-"*85)
        for c in clientes:
            print(f"{c.id:<4} | {c.nome:<29} | {Data_Utils.data_para_sting(c.data_nascimento):<15} | {c.idade:<10} | {c.limite_credito:<15}")
        print("-"*85)
